<div id="navbar1" class="sidebar" data-color="blue">
  ...
  -->
    ...
    <div class="sidebar-wrapper" id="sidebar-wrapper">
        <ul class="nav">
            <% if(!themeConfig.sidebar || themeConfig.sidebar.keepDefault) { %>
                <li class="<% if(now == 'index'){ %>active<% } %>">
                    <%
                    let homePage = "/";
                    if(themeConfig.landingPage?.enabled) homePage = "/dash";
                    %>
                    <a href="<%-homePage%>">
            ...
            <% if(themeConfig.sidebar){ %>
                <% themeConfig.sidebar.list.forEach(li => { %>
                    <li class="<% if(now == li.id){ %>active<% } %>">
                        <a href="<%- li.link %>">
                            <%- li.icon %>
                            <p style="margin-left: 40px;"><%- li.title %></p>
                        </a>
                    </li>
                <% }) %>
            <% } %>
                        ...
                        <span style="cursor: pointer;" class="nav-link waves-effect waves-dark profile-pic"
                              data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><img
                                    ...
                                    class="profile-pic"><b><%= req.session.user.username %> </b><span
                                        style="cursor:pointer;color:<%- themeConfig.information.mainColor %>;"
                                        onclick="window.location = '/discord/logout'"> &nbsp;&nbsp;Log out</span> &nbsp;&nbsp;</span>
                </a>
                <% }else{ %>
                <span style="cursor: pointer;" class="nav-link waves-effect waves-dark profile-pic"
                      data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><img
